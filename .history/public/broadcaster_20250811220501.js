const socket = io();
let localStream = null; // Combined screen + mic (broadcaster)
let micStream = null;   // Mic audio only stream
const pcs = {};
const config = {
...
startBtn.onclick = async () => {
  try {
    // Get display media (screen + audio if any)
...
async function addMicTracks() {
  try {
    ...
    // Add mic audio track(s) to localStream (if not already there)
    micStream.getAudioTracks().forEach(track => {
      localStream.addTrack(track);
    });

    // Add mic tracks to all existing peer connections and renegotiate
...
async function removeMicTracks() {
  ...
  // Remove mic tracks from all peer connections and renegotiate
  for (const [viewerId, pc] of Object.entries(pcs)) {
...
socket.on('new-viewer', async ({ viewerId }) => {
  ...
  // Add all tracks (screen + mic) to the connection
  localStream.getTracks().forEach(track => pc.addTrack(track, localStream));

  // When receiving tracks (e.g., viewer mic) - play them for broadcaster
  pc.ontrack = (event) => {
    const stream = event.streams[0];
    let audioEl = document.getElementById(`viewerAudio_${viewerId}`);
    if (!audioEl) {
      audioEl = document.createElement('audio');
      audioEl.id = `viewerAudio_${viewerId}`;
      audioEl.autoplay = true;
      audioEl.style.display = 'none'; // hide if you want
      document.body.appendChild(audioEl);
    }
    audioEl.srcObject = stream;
    audioEl.play().catch(e => console.warn('Failed to play viewer audio:', e));
  };
...
socket.on('peer-left', ({ id }) => {
  if (pcs[id]) {
    pcs[id].close();
    delete pcs[id];
    // Remove corresponding viewer audio element
    const audioEl = document.getElementById(`viewerAudio_${id}`);
